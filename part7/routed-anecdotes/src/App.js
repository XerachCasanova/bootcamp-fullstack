...
import {useFieldAnecdotes, useReset} from './hooks'
import {
...
const CreateNew = (props) => {
  ...
  const content = useFieldAnecdotes('text')
  const author = useFieldAnecdotes('text')
  const info = useFieldAnecdotes('text')
  ...
  const button = useReset('reset', fieldsToReset)
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit">create</button><button {...button} >reset</button>
      </form>
    </div>
  )
...
const App = (props) => {
  ...
  if (notification) setTimeout(() => setNotification(''), 10000)
